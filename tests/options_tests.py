...
class OptionsTest(unittest.TestCase):
    ...
    def test_encrypt_decrypt(self):
        ...
        encrypted_source = options.encrypt(
            key, source.encode("utf-8")).encode("utf-8")
        decrypted_source = options.decrypt(key, encrypted_source)
    ...
    def test_save_get_store(self):
        options.add_property(self.prop_name, self.prop_value, self.password)
        new_store_value = ""
        new_store = options.encrypt(self.password.encode(
            "utf-8"), new_store_value.encode("utf-8"))
